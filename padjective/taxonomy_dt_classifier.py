"""Train decision tree classifier for taxonomy prediction.

This module provides a decision tree classifier that predicts taxonomy IDs from
product tags. Decision trees are interpretable models that make decisions based
on a series of if-then-else rules derived from the features.

Key characteristics:
- Uses ALL available tags (like ULR/UNN, unlike constrained PCLR/PCNN)
- Tree depth controls model complexity (number of decision rules)
- Provides feature importance rankings
- Highly interpretable - can visualize the decision path
"""
...
def train_decision_tree(
    ...
    *,
    ...
    random_state: int = 42,
) -> tuple[DecisionTreeClassifier, TrainingStats]:
...
def main() -> None:
    """Command-line interface for training decision tree classifier."""
    parser = argparse.ArgumentParser(
        description="Train decision tree classifier to predict product taxonomy from tags"
    )
    parser.add_argument(
        "--dsn",
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("build/decision_tree"),
    ...
    # Load data - NO max-tags constraint (uses ALL tags)
    ...
    if args.fold is not None:
        ...
        # Train model
        model, stats = train_decision_tree(
            X_train,
            y_train,
        ...
        try:
            with save_conn.cursor() as cur:
                ...
                cur.execute(
                    sql.SQL(
                        """
                        CREATE TABLE IF NOT EXISTS {schema}.taxonomy_dt_fold_results (
                ...
                cur.execute(
                    sql.SQL(
                        """
                        CREATE TABLE IF NOT EXISTS {schema}.taxonomy_dt_predictions (
                ...
                cur.execute(
                    sql.SQL(
                        """
                        CREATE TABLE IF NOT EXISTS {schema}.taxonomy_dt_feature_importance (
                            cv_fold INTEGER NOT NULL,
                            tag TEXT NOT NULL,
                ...
                cur.execute(
                    sql.SQL("DELETE FROM {schema}.taxonomy_dt_fold_results WHERE cv_fold = %s").format(
                ...
                cur.execute(
                    sql.SQL(
                        """
                        INSERT INTO {schema}.taxonomy_dt_fold_results
                ...
                cur.execute(
                    sql.SQL("DELETE FROM {schema}.taxonomy_dt_predictions WHERE cv_fold = %s").format(
                        schema=sql.Identifier(args.results_schema)
                    ),
                    (args.fold,)
                )
                ...
                for idx in range(len(y_test)):
                    ...
                    cur.execute(
                        sql.SQL(
                            """
                            INSERT INTO {schema}.taxonomy_dt_predictions
                ...
                cur.execute(
                    sql.SQL("DELETE FROM {schema}.taxonomy_dt_feature_importance WHERE cv_fold = %s").format(
                        schema=sql.Identifier(args.results_schema)
                    ),
                    (args.fold,)
                )
                ...
                importance_indices = np.argsort(importances)[::-1]  # Sort by importance descending

                for rank, idx in enumerate(importance_indices):
                    if importances[idx] > 0:  # Only save non-zero importances
                        cur.execute(
                            sql.SQL(
                                """
                                INSERT INTO {schema}.taxonomy_dt_feature_importance
                                (cv_fold, tag, importance, rank)
                                VALUES (%s, %s, %s, %s)
                                """
                            ).format(schema=sql.Identifier(args.results_schema)),
                            (args.fold, feature_names[idx], float(importances[idx]), rank + 1)
                        )

            save_conn.commit()
            print(f"\nResults saved to {args.results_schema}.taxonomy_dt_fold_results")
        finally:
            save_conn.close()
