...
def render_model_comparison_tex(bundle: dict[str, Any]) -> str:
    lines = [
        ...
        "\\begin{tabular}{lrrr}",
        "\\toprule",
        "\\makecell{\\textbf{Model}} & \\makecell{\\textbf{Non-zero}\\\\\\textbf{params.}} & \\makecell{\\textbf{Avg active}\\\\\\textbf{params.}} & \\makecell{\\textbf{Mean $p$-adic}\\\\\\textbf{loss}} \\\\",
        "\\midrule",
    ]
    for row in model_rows_frame(bundle).to_dict("records"):
        lines.append(
...
def render_umllr_ablation_tex(bundle: dict[str, Any]) -> str:
    lines = [
        ...
        "\\toprule",
        "\\textbf{Strategy} & \\textbf{Mean loss} & $\\Delta$ vs battle & \\textbf{Wins} & \\textbf{Exact acc.} & \\textbf{Prefix-2 acc.} \\\\",
        "\\midrule",
    ]
    for row in ablation_strategy_frame(bundle).to_dict("records"):
        lines.append(
