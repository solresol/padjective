...
def _write_defective_taxonomy_page(output_dir: Path, dataset: data_access.ProductDataset) -> Path:
    ...
    if defective_rows:
        ...
        count_msg = f"<p>{len(defective_rows)} products have defective taxonomy paths (missing hierarchical separators like '/' or '>').</p>"
    else:
        defective_table = "<p>No products with defective taxonomy paths found.</p>"
        count_msg = "<p>All products have valid hierarchical taxonomy paths.</p>"
...
<body class="dataset-page">
  ...
  <section>
    ...
    {count_msg}
    <p>These products have taxonomy paths that don't contain hierarchical separators ('/', '>', or '|').
    They typically appear as numeric codes like "1.1.4" instead of proper hierarchical paths like
    "Apparel &amp; Accessories / Clothing / T-Shirts". These products are excluded from model training
    to ensure consistent data quality.</p>
