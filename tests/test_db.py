...
class RecordingConnection:
    def __init__(self, table_exists: bool, *, table_owned: bool = True) -> None:
        self.table_exists = table_exists
        self.table_owned = table_owned
...
class RecordingCursor:
    ...
    def fetchone(self) -> tuple[int] | None:
        ...
        if self.connection.table_exists:
            return (self.connection.table_owned,)
        return None
...
def test_ensure_table_skips_creation_when_table_exists() -> None:
    ...
    assert "pg_catalog.pg_tables" in statement
    assert params == ("padjective", "example")


def test_ensure_table_skips_index_ddl_when_existing_table_is_not_owned() -> None:
    conn = RecordingConnection(table_exists=True, table_owned=False)
...
def test_ensure_table_reconciles_indexes_when_existing_table_is_owned() -> None:
    conn = RecordingConnection(table_exists=True, table_owned=True)
    ...
    index_statement, index_params = conn.statements[1]
    assert index_statement.startswith("CREATE INDEX IF NOT EXISTS example_id_idx")
